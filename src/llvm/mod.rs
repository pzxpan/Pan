...
pub fn AddFunctionAttrStringValue<'a>(llfn: &'a Value, idx: AttributePlace, attr: &CStr, value: &CStr) {
    unsafe {
        LLVMRustAddFunctionAttrStringValue(llfn, idx.as_uint(), attr.as_ptr(), value.as_ptr())
    }
}

pub fn AddFunctionAttrString<'a>(llfn: &'a Value, idx: AttributePlace, attr: &CStr) {
    unsafe {
...
pub fn SetInstructionCallConv<'a>(instr: &'a Value, cc: CallConv) {
    unsafe {
        LLVMSetInstructionCallConv(instr, cc as c_uint);
    }
}
pub fn SetFunctionCallConv<'a>(fn_: &'a Value, cc: CallConv) {
    unsafe {
...
pub fn SetUniqueComdat<'a>(llmod: &Module, val: &'a Value) {
    unsafe {
...
pub fn UnsetComdat<'a>(val: &'a Value) {
    unsafe {
        LLVMRustUnsetComdat(val);
    }
}

pub fn SetUnnamedAddress<'a>(global: &'a Value, unnamed: UnnamedAddr) {
    unsafe {
        LLVMSetUnnamedAddress(global, unnamed);
    }
}

pub fn set_thread_local_mode<'a>(global: &'a Value, mode: ThreadLocalMode) {
    unsafe {
...
impl<'a> OperandBundleDef<'a> {
    pub fn new(name: &str, vals: &[&'a Value]) -> Self {
        let name = small_c_str::SmallCStr::new(name);
        let def = unsafe {
...
impl<'a> Drop for OperandBundleDef<'a> {
    fn drop(&mut self) {
        unsafe {
