...
use pan_ast::ptr::P;
use pan_ast::{self as ast, BinOpKind, EnumDef, Expr, Generics, PatKind};
use pan_ast::{GenericArg, GenericParamKind, VariantData};
use pan_attr as attr;
use pan_data_structures::map_in_place::MapInPlace;
use pan_expand::base::{Annotatable, ExtCtxt};
...
impl<'a> TraitDef<'a> {
    ...
    fn create_derived_impl(
        ...
        let unused_qual = {
            let word = pan_ast::attr::mk_nested_word_item(Ident::new(
                sym::unused_qualifications,
                self.span,
            ));
            let list = pan_ast::attr::mk_list_item(Ident::new(sym::allow, self.span), vec![word]);
            cx.attribute(list)
        };
