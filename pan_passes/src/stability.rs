...
use pan_ast::Attribute;
use pan_attr::{self as attr, ConstStability, Stability};
use pan_data_structures::fx::{FxHashMap, FxHashSet};
use pan_errors::struct_span_err;
use pan_hir as hir;
use pan_hir::def::{DefKind, Res};
use pan_hir::def_id::{DefId, LocalDefId, CRATE_DEF_INDEX, LOCAL_CRATE};
use pan_hir::intravisit::{self, NestedVisitorMap, Visitor};
use pan_hir::{Generics, HirId, Item, StructField, TraitRef, Ty, TyKind, Variant};
use pan_middle::hir::map::Map;
use pan_middle::middle::privacy::AccessLevels;
use pan_middle::middle::stability::{DeprecationEntry, Index};
use pan_middle::ty::{self, query::Providers, TyCtxt};
use pan_session::lint;
use pan_session::lint::builtin::{INEFFECTIVE_UNSTABLE_TRAIT_IMPL, USELESS_DEPRECATED};
...
impl<'a, 'tcx> Annotator<'a, 'tcx> {
        ...
        F: FnOnce(&mut Self),
    {
                    ...
                    lint.build("this `#[deprecated]` annotation has no effect")
                        .span_suggestion_short(
                            *span,
                            ...
                            String::new(),
                            pan_errors::Applicability::MachineApplicable,
                        )
                        .emit()
                });
            }
        ...
        if let Some((pan_attr::Deprecation { is_since_rustc_version: true, .. }, span)) = &depr {
            if stab.is_none() {
                struct_span_err!(
                    self.tcx.sess,
                    *span,
