...
#![doc(html_root_url = "https://doc.rust-lang.org/nightly/nightly-pan/")]
...
use pan_data_structures::AtomicRef;
use pan_macros::HashStable_Generic;
use pan_serialize::{Decodable, Decoder, Encodable, Encoder};
...
use pan_data_structures::sync::{Lock, Lrc};
...
pub enum RealFileName {
    ...
    Devirtualized {
        ...
        local_path: PathBuf,
        /// `virtual_name` is the stable path pan will store internally within
        /// build artifacts.
...
pub enum FileName {
    ...
    /// Command line.
    Anon(u64),
    /// Hack in `src/libpan_ast/parse.rs`.
    // FIXME(jseyfried)
    MacroExpansion(u64),
...
/// (see `pan_interface::callbacks::span_debug1`). However, some parts
/// of the compiler (e.g. `pan_parse`) may debug-print `Span`s before
...
pan_data_structures::impl_stable_hash_via_hash!(SourceFileHashAlgorithm);
...
impl<S: pan_serialize::Encoder> Encodable<S> for BytePos {
    fn encode(&self, s: &mut S) -> Result<(), S::Error> {
        s.emit_u32(self.0)
    }
}

impl<D: pan_serialize::Decoder> Decodable<D> for BytePos {
...
/// instead of implementing everything in pan_middle.
