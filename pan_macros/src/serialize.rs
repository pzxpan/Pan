...
pub fn decodable_derive(mut s: synstructure::Structure<'_>) -> proc_macro2::TokenStream {
    let decoder_ty = quote! { __D };
    s.add_impl_generic(parse_quote! {#decoder_ty: ::pan_serialize::Decoder});
    s.add_bounds(synstructure::AddBounds::Generics);
...
fn decodable_body(
    ...
    let decode_body = match s.variants() {
        [vi] => {
            ...
            quote! {
                ::pan_serialize::Decoder::read_struct(
                    __decoder,
                    #ty_name,
                    #n_fields,
        ...
        variants => {
            ...
            quote! {
                ::pan_serialize::Decoder::read_enum(
                    __decoder,
                    #ty_name,
                    |__decoder| {
                        ::pan_serialize::Decoder::read_enum_variant(
                            __decoder,
                            &[#names],
                            |__decoder, __variant_idx| {
                                match __variant_idx {
                                    ...
                                    _ => return ::std::result::Result::Err(
                                        ::pan_serialize::Decoder::error(__decoder, #message)),
                                }
                            })
                    }
                )
    ...
    s.bound_impl(
        quote!(::pan_serialize::Decodable<#decoder_ty>),
        quote! {
            fn decode(
                __decoder: &mut #decoder_ty,
            ) -> ::std::result::Result<Self, <#decoder_ty as ::pan_serialize::Decoder>::Error> {
                #decode_body
            }
        },
    )
...
fn decode_field(field: &syn::Field, index: usize, is_struct: bool) -> proc_macro2::TokenStream {
    ...
    quote! {
        match ::pan_serialize::Decoder::#decode_method(
...
pub fn encodable_derive(mut s: synstructure::Structure<'_>) -> proc_macro2::TokenStream {
    let encoder_ty = quote! { __E };
    s.add_impl_generic(parse_quote! { #encoder_ty: ::pan_serialize::Encoder});
    s.add_bounds(synstructure::AddBounds::Generics);
...
fn encodable_body(
    ...
    let encode_body = match s.variants() {
        [_] => {
            ...
            let encode_inner = s.each_variant(|vi| {
                vi.bindings()
                    ...
                    .map(|binding| {
                        ...
                        let result = quote! {
                            match ::pan_serialize::Encoder::emit_struct_field(
                                __encoder,
                                #field_name,
                                #field_idx,
                                |__encoder|
                                ::pan_serialize::Encodable::<#encoder_ty>::encode(#bind_ident, __encoder),
                            ) {
            ...
            quote! {
                ::pan_serialize::Encoder::emit_struct(__encoder, #ty_name, #field_idx, |__encoder| {
        ...
        _ => {
            ...
            let encode_inner = s.each_variant(|vi| {
                ...
                let encode_fields: TokenStream = vi
                    ...
                    .map(|binding| {
                        let bind_ident = &binding.binding;
                        let result = quote! {
                            match ::pan_serialize::Encoder::emit_enum_variant_arg(
                                __encoder,
                                #field_idx,
                                |__encoder|
                                ::pan_serialize::Encodable::<#encoder_ty>::encode(#bind_ident, __encoder),
                ...
                let result = quote! { ::pan_serialize::Encoder::emit_enum_variant(
                    __encoder,
                   #variant_name,
                   #variant_idx,
                   #field_idx,
            ...
            quote! {
                ::pan_serialize::Encoder::emit_enum(__encoder, #ty_name, |__encoder| {
                    match *self {
                        #encode_inner
                    }
                })
    ...
    s.bound_impl(
        quote!(::pan_serialize::Encodable<#encoder_ty>),
        quote! {
            fn encode(
                &self,
                __encoder: &mut #encoder_ty,
            ) -> ::std::result::Result<(), <#encoder_ty as ::pan_serialize::Encoder>::Error> {
                #lints
                #encode_body
            }
        },
