...
use pan_hir as hir;
use pan_middle::mir::interpret::{InterpError, InterpErrorInfo};
use pan_middle::ty;
use pan_middle::ty::layout::TyAndLayout;
use pan_span::symbol::{sym, Symbol};
use pan_target::abi::{Abi, LayoutOf, Scalar, Size, VariantIdx, Variants};
...
macro_rules! throw_validation_failure {
    ($where:expr, { $( $what_fmt:expr ),+ } $( expected { $( $expected_fmt:expr ),+ } )?) => {{
        let msg = pan_middle::ty::print::with_no_trimmed_paths(|| {
