...
use pan_ast as ast;
use pan_ast::util::parser;
use pan_ast::{ExprKind, StmtKind};
use pan_ast_pretty::pprust;
use pan_data_structures::fx::FxHashMap;
use pan_errors::{pluralize, Applicability};
use pan_feature::{AttributeType, BuiltinAttribute, BUILTIN_ATTRIBUTE_MAP};
use pan_hir as hir;
use pan_hir::def::{DefKind, Res};
use pan_hir::def_id::DefId;
use pan_middle::ty::adjustment;
use pan_middle::ty::{self, Ty};
use pan_session::lint::builtin::UNUSED_ATTRIBUTES;
use pan_span::symbol::Symbol;
use pan_span::symbol::{kw, sym};
use pan_span::{BytePos, Span, DUMMY_SP};
...
trait UnusedDelimLint {
        ...
        let lhs_needs_parens = {
            ...
            loop {
                if let ExprKind::Binary(_, lhs, _rhs) = &innermost.kind {
                    innermost = lhs;
                    if !pan_ast::util::classify::expr_requires_semi_to_be_stmt(innermost) {
                        break true;
                    }
                } else {
                    break false;
    ...
    fn check_expr(&mut self, cx: &EarlyContext<'_>, e: &ast::Expr) {
        use pan_ast::ExprKind::*;
        let (value, ctx, followed_by_block, left_pos, right_pos) = match e.kind {
            // Do not lint `unused_braces` in `if let` expressions.
            If(ref cond, ref block, ..)
                if !matches!(cond.kind, Let(_, _)) || Self::LINT_EXPR_IN_PATTERN_MATCHING_CTX =>
            {
                let left = e.span.lo() + pan_span::BytePos(2);
            ...
            While(ref cond, ref block, ..)
                if !matches!(cond.kind, Let(_, _)) || Self::LINT_EXPR_IN_PATTERN_MATCHING_CTX =>
            {
                let left = e.span.lo() + pan_span::BytePos(5);
                let right = block.span.lo();
            ...
            Ret(Some(ref value)) => {
                let left = e.span.lo() + pan_span::BytePos(3);
                ...
                if !Self::is_expr_delims_necessary(inner, followed_by_block)
                    ...
                    && (ctx != UnusedDelimsCtx::LetScrutineeExpr
                        || !matches!(inner.kind, ast::ExprKind::Binary(
                                pan_span::source_map::Spanned { node, .. },
                                _,
                                _,
                            ) if node.lazy()))
                {
