...
use pan_ast::{AssocTyConstraint, AssocTyConstraintKind, NodeId};
use pan_ast::{PatKind, RangeEnd, VariantData};
use pan_errors::struct_span_err;
use pan_feature::{AttributeGate, BUILTIN_ATTRIBUTE_MAP};
use pan_feature::{Features, GateIssue};
use pan_session::parse::{feature_err, feature_err_issue};
...
fn check_incompatible_features(sess: &Session) {
    ...
    for (f1, f2) in pan_feature::INCOMPATIBLE_FEATURES
        .iter()
