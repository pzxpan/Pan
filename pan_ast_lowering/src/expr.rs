...
use pan_span::hygiene::ForLoopLoc;
use pan_span::source_map::{respan, DesugaringKind, Span, Spanned};
...
impl<'hir> LoweringContext<'_, 'hir> {
    ...
    fn extract_tuple_struct_path<'a>(&mut self, expr: &'a Expr) -> Option<&'a Path> {
        ...
        if let ExprKind::Path(None, path) = &expr.kind {
            println!("start resolved:{:?}", expr.id);
            // Does the path resolves to something disallowed in a tuple struct/variant pattern?
            if let Some(partial_res) = self.resolver.get_partial_res(expr.id) {
                if partial_res.unresolved_segments() == 0
                    && !partial_res.base_res().expected_in_tuple_struct_pat()
                {
                    println!("not resolved:{:?}", expr.id);
                    return None;
                }
            }
            println!("resovle resolved:{:?}", expr.id);
            return Some(path);
        }
        println!("not found resovle resolved:{:?}", expr.id);
        None
    }
