...
use pan_ast::ast;
use pan_middle::traits::{ChalkEnvironmentAndGoal, ChalkRustInterner as RustInterner};
use pan_middle::ty::fold::TypeFolder;
use pan_middle::ty::subst::{GenericArg, GenericArgKind, SubstsRef};
use pan_middle::ty::{self, Binder, Region, RegionKind, Ty, TyCtxt, TypeFoldable, TypeVisitor};
use pan_span::def_id::DefId;

use chalk_ir::{FnSig, ForeignDefId};
use pan_hir::Unsafety;
...
impl<'tcx> LowerInto<'tcx, chalk_ir::TraitRef<RustInterner<'tcx>>>
    for pan_middle::ty::TraitRef<'tcx>
{
...
impl<'tcx> LowerInto<'tcx, chalk_ir::AliasEq<RustInterner<'tcx>>>
    for pan_middle::ty::ProjectionPredicate<'tcx>
{
...
impl<'tcx> LowerInto<'tcx, chalk_ir::Mutability> for ast::Mutability {
    fn lower_into(self, _interner: &RustInterner<'tcx>) -> chalk_ir::Mutability {
        match self {
            pan_ast::Mutability::Mut => chalk_ir::Mutability::Mut,
            pan_ast::Mutability::Not => chalk_ir::Mutability::Not,
        }
    }
}
...
crate struct ParamsSubstitutor<'tcx> {
    tcx: TyCtxt<'tcx>,
    binder_index: ty::DebruijnIndex,
    list: Vec<pan_middle::ty::ParamTy>,
    next_ty_placeholder: usize,
    crate params: pan_data_structures::fx::FxHashMap<usize, pan_middle::ty::ParamTy>,
...
impl<'tcx> ParamsSubstitutor<'tcx> {
    crate fn new(tcx: TyCtxt<'tcx>, next_ty_placeholder: usize) -> Self {
        ParamsSubstitutor {
            ...
            list: vec![],
            next_ty_placeholder,
            params: pan_data_structures::fx::FxHashMap::default(),
