...
use pan_codegen_ssa::{CompiledModule, ModuleCodegen};
use pan_data_structures::small_c_str::SmallCStr;
use pan_errors::{FatalError, Handler, Level};
use pan_fs_util::{link_or_copy, path_to_c_string};
...
use pan_span::InnerSpan;
use pan_target::spec::{CodeModel, RelocModel};
use tracing::debug;
...
pub fn llvm_err(handler: &pan_errors::Handler, msg: &str) -> FatalError {
...
unsafe fn embed_bitcode(
    ...
    let llglobal = llvm::LLVMAddGlobal(
        ...
        "pan.embedded.module\0".as_ptr().cast(),
    );
    ...
    let llglobal = llvm::LLVMAddGlobal(
        llmod,
        common::val_ty(llconst),
        "pan.embedded.cmdline\0".as_ptr().cast(),
    );
    llvm::LLVMSetInitializer(llglobal, llconst);
