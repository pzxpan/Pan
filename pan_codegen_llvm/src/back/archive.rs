...
use pan_codegen_ssa::back::archive::{find_library, ArchiveBuilder};
use pan_codegen_ssa::{looks_like_rust_object_file, METADATA_FILENAME};
...
fn archive_config<'a>(sess: &'a Session, output: &Path, input: Option<&Path>) -> ArchiveConfig<'a> {
    use pan_codegen_ssa::back::link::archive_search_paths;
    ArchiveConfig {
        sess,
