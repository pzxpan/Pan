...
use pan_codegen_ssa::mir::debuginfo::VariableKind::*;
...
use pan_codegen_ssa::debuginfo::type_names;
use pan_codegen_ssa::mir::debuginfo::{DebugScope, FunctionDebugContext, VariableKind};
use pan_codegen_ssa::traits::*;
use pan_data_structures::fx::{FxHashMap, FxHashSet};
use pan_data_structures::sync::Lrc;
use pan_hir::def_id::{DefId, DefIdMap, LOCAL_CRATE};
use pan_index::vec::IndexVec;
use pan_middle::mir;
use pan_middle::ty::layout::HasTyCtxt;
use pan_middle::ty::subst::{GenericArgKind, SubstsRef};
use pan_middle::ty::{self, Instance, ParamEnv, Ty, TypeFoldable};
use pan_session::config::{self, DebugInfo};
use pan_span::symbol::Symbol;
use pan_span::{self, BytePos, Pos, SourceFile, SourceFileAndLine, Span};
use pan_target::abi::{LayoutOf, Primitive, Size};
    ...
    fn dbg_scope_fn(
        ...
        fn get_function_signature<'ll, 'tcx>(
            ...
            signature.push(if fn_abi.ret.is_ignore() {
                ...
                Some(type_metadata(cx, fn_abi.ret.layout.ty, pan_span::DUMMY_SP))
            });
            ...
            if cx.sess().target.is_like_msvc {
                ...
                signature.extend(fn_abi.args.iter().map(|arg| {
                    ...
                    Some(type_metadata(cx, t, pan_span::DUMMY_SP))
                }));
            } else {
                signature.extend(
                    fn_abi
                        .args
                        .iter()
                        .map(|arg| Some(type_metadata(cx, arg.layout.ty, pan_span::DUMMY_SP))),
                );
            }
                ...
                if cx.tcx.trait_id_of_impl(impl_def_id).is_none() {
                    ...
                    match impl_self_ty.kind() {
                        ty::Adt(def, ..) if !def.is_box() => {
                            ...
                            {
                                Some(type_metadata(cx, impl_self_ty, pan_span::DUMMY_SP))
                            } else {
