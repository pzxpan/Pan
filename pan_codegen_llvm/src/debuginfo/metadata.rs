...
use pan_ast as ast;
use pan_codegen_ssa::traits::*;
use pan_data_structures::const_cstr;
use pan_data_structures::fingerprint::Fingerprint;
use pan_data_structures::fx::FxHashMap;
use pan_data_structures::stable_hasher::{HashStable, StableHasher};
use pan_fs_util::path_to_c_string;
use pan_hir::def::CtorKind;
use pan_hir::def_id::{DefId, LOCAL_CRATE};
use pan_index::vec::{Idx, IndexVec};
use pan_middle::ich::NodeIdHashingMode;
use pan_middle::mir::{self, Field, GeneratorLayout};
use pan_middle::ty::layout::{self, IntegerExt, PrimitiveExt, TyAndLayout};
use pan_middle::ty::subst::GenericArgKind;
use pan_middle::ty::Instance;
use pan_middle::ty::{self, AdtKind, GeneratorSubsts, ParamEnv, Ty, TyCtxt};
use pan_middle::{bug, span_bug};
use pan_session::config::{self, DebugInfo};
use pan_span::symbol::{Interner, Symbol};
use pan_span::{self, SourceFile, SourceFileHash, Span};
use pan_target::abi::{Abi, Align, HasDataLayout, Integer, LayoutOf, TagEncoding};
use pan_target::abi::{Int, Pointer, F32, F64};
use pan_target::abi::{Primitive, Size, VariantIdx, Variants};
use tracing::debug;
...
fn trait_pointer_metadata(
    ...
    let member_descriptions = vec![
        ...
        MemberDescription {
            ...
            type_metadata: type_metadata(cx, vtable_field.ty, pan_span::DUMMY_SP),
            ...
            let (hash_kind, hash_value) = match hash {
                Some(hash) => {
                    let kind = match hash.kind {
                        pan_span::SourceFileHashAlgorithm::Md5 => llvm::ChecksumKind::MD5,
                        pan_span::SourceFileHashAlgorithm::Sha1 => llvm::ChecksumKind::SHA1,
                        pan_span::SourceFileHashAlgorithm::Sha256 => llvm::ChecksumKind::SHA256,
                    };
...
pub fn compile_unit_metadata(
    ...
    let rustc_producer =
        format!("pan version {}", option_env!("CFG_VERSION").expect("CFG_VERSION"),);
        ...
        match cached_discriminant_type_metadata {
            ...
            None => {
                ...
                let discriminant_base_type_metadata =
                    type_metadata(cx, discr.to_ty(tcx), pan_span::DUMMY_SP);
                ...
                .filter_map(|(kind, name)| {
                    if let GenericArgKind::Type(ty) = kind.unpack() {
                        ...
                        let actual_type_metadata =
                            type_metadata(cx, actual_type, pan_span::DUMMY_SP);
...
pub fn create_vtable_metadata(cx: &CodegenCx<'ll, 'tcx>, ty: Ty<'tcx>, vtable: &'ll Value) {
    ...
    let type_metadata = type_metadata(cx, ty, pan_span::DUMMY_SP);
