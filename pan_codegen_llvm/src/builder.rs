...
use pan_codegen_ssa::common::{IntPredicate, RealPredicate, TypeKind};
use pan_codegen_ssa::mir::operand::{OperandRef, OperandValue};
use pan_codegen_ssa::mir::place::PlaceRef;
use pan_codegen_ssa::traits::*;
use pan_codegen_ssa::MemFlags;
use pan_data_structures::const_cstr;
use pan_data_structures::small_c_str::SmallCStr;
use pan_hir::def_id::DefId;
use pan_middle::ty::layout::TyAndLayout;
use pan_middle::ty::{self, Ty, TyCtxt};
use pan_span::{sym, Span};
use pan_target::abi::{self, Align, Size};
use pan_target::spec::{HasTargetSpec, Target};
use std::borrow::Cow;
use std::ffi::CStr;
...
impl BuilderMethods<'a, 'tcx> for Builder<'a, 'll, 'tcx> {
    ...
    fn atomic_load(
        &mut self,
        ptr: &'ll Value,
        order: pan_codegen_ssa::common::AtomicOrdering,
        size: Size,
    ) -> &'ll Value {
        unsafe {
    ...
    fn atomic_cmpxchg(
        ...
        src: &'ll Value,
        order: pan_codegen_ssa::common::AtomicOrdering,
        failure_order: pan_codegen_ssa::common::AtomicOrdering,
        weak: bool,
    ) -> &'ll Value {
    ...
    fn atomic_rmw(
        &mut self,
        op: pan_codegen_ssa::common::AtomicRmwBinOp,
        dst: &'ll Value,
        src: &'ll Value,
        order: pan_codegen_ssa::common::AtomicOrdering,
    ) -> &'ll Value {
    ...
    fn atomic_fence(
        &mut self,
        order: pan_codegen_ssa::common::AtomicOrdering,
        scope: pan_codegen_ssa::common::SynchronizationScope,
    ) {
        unsafe {
