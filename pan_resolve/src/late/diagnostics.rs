...
use pan_ast::visit::FnKind;
use pan_ast::{self as ast, Expr, ExprKind, Item, ItemKind, NodeId, Path, Ty, TyKind};
use pan_ast_pretty::pprust::path_segment_to_string;
use pan_data_structures::fx::FxHashSet;
use pan_errors::{pluralize, struct_span_err, Applicability, DiagnosticBuilder};
use pan_hir as hir;
use pan_hir::def::Namespace::{self, *};
use pan_hir::def::{self, CtorKind, CtorOf, DefKind};
use pan_hir::def_id::{DefId, CRATE_DEF_INDEX, LOCAL_CRATE};
use pan_hir::PrimTy;
...
use pan_span::{BytePos, MultiSpan, Span, DUMMY_SP};
...
impl<'a: 'ast, 'ast> LateResolutionVisitor<'a, '_, 'ast> {
    ...
    pub(crate) fn smart_resolve_report_errors(
        ...
        if is_self_type(path, ns) {
            err.code(pan_errors::error_code!(E0411));
            err.span_label(
                span,
        ...
        if is_self_value(path, ns) {
            ...
            err.code(pan_errors::error_code!(E0424));
        ...
        if let Some(err_code) = &err.code {
            if err_code == &pan_errors::error_code!(E0425) {
            ...
            (Res::Def(DefKind::Struct, def_id), _) if ns == ValueNS => {
                    ...
                    field_spans.filter(|spans| spans.len() > 0 && fields.len() == spans.len())
                {
                    ...
                    if non_visible_spans.len() > 0 {
                        let mut m: pan_span::MultiSpan = non_visible_spans.clone().into();
                        non_visible_spans
                            .into_iter()
