...
pub use pan_hir::def::{Namespace, PerNS};

use Determinacy::*;

use pan_arena::{DroplessArena, TypedArena};
use pan_ast::node_id::NodeMap;
use pan_ast::unwrap_or;
use pan_ast::visit::{self, Visitor};
use pan_ast::{self as ast, FloatTy, IntTy, NodeId, UintTy};
use pan_ast::{Crate, CRATE_NODE_ID};
use pan_ast::{ItemKind, Path};
use pan_ast_lowering::ResolverAstLowering;
use pan_ast_pretty::pprust;
use pan_data_structures::fx::{FxHashMap, FxHashSet, FxIndexMap};
use pan_data_structures::ptr_key::PtrKey;
use pan_data_structures::sync::Lrc;
use pan_errors::{struct_span_err, Applicability, DiagnosticBuilder};
use pan_expand::base::SyntaxExtension;
use pan_hir::def::Namespace::*;
use pan_hir::def::{self, CtorOf, DefKind, NonMacroAttrKind, PartialRes};
use pan_hir::def_id::{CrateNum, DefId, DefIdMap, LocalDefId, CRATE_DEF_INDEX};
use pan_hir::definitions::{DefKey, DefPathData, Definitions};
use pan_hir::PrimTy::{self, Bool, Char, Float, Int, Str, Uint};
use pan_hir::TraitCandidate;
use pan_index::vec::IndexVec;
use pan_metadata::creader::{CStore, CrateLoader};
use pan_middle::hir::exports::ExportMap;
use pan_middle::middle::cstore::{CrateStore, MetadataLoaderDyn};
use pan_middle::ty::query::Providers;
use pan_middle::ty::{self, DefIdTree, ResolverOutputs};
use pan_middle::{bug, span_bug};
use pan_session::lint;
use pan_session::lint::{BuiltinLintDiagnostics, LintBuffer};
use pan_session::Session;
use pan_span::hygiene::{ExpnId, ExpnKind, MacroKind, SyntaxContext, Transparency};
...
impl<'a> Resolver<'a> {
    pub fn new(
        ...
        assert_eq!(def_id_to_span.push(pan_span::DUMMY_SP), root);
