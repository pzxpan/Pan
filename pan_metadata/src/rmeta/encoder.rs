...
use pan_ast as ast;
use pan_data_structures::fingerprint::{Fingerprint, FingerprintEncoder};
use pan_data_structures::fx::{FxHashMap, FxHashSet, FxIndexSet};
use pan_data_structures::stable_hasher::StableHasher;
use pan_data_structures::sync::{join, Lrc};
use pan_hir as hir;
use pan_hir::def::CtorKind;
use pan_hir::def_id::{CrateNum, DefId, DefIndex, LocalDefId, CRATE_DEF_INDEX, LOCAL_CRATE};
use pan_hir::definitions::DefPathData;
use pan_hir::intravisit::{self, NestedVisitorMap, Visitor};
use pan_hir::itemlikevisit::{ItemLikeVisitor, ParItemLikeVisitor};
use pan_hir::lang_items;
use pan_hir::{AnonConst, GenericParamKind};
use pan_index::bit_set::GrowableBitSet;
use pan_index::vec::Idx;
use pan_middle::hir::map::Map;
use pan_middle::middle::cstore::{EncodedMetadata, ForeignModule, LinkagePreference, NativeLib};
use pan_middle::middle::dependency_format::Linkage;
use pan_middle::middle::exported_symbols::{
    metadata_symbol_name, ExportedSymbol, SymbolExportLevel,
};
use pan_middle::mir::interpret;
use pan_middle::traits::specialization_graph;
use pan_middle::ty::codec::TyEncoder;
use pan_middle::ty::{self, SymbolName, Ty, TyCtxt};
use pan_serialize::{opaque, Encodable, Encoder};
use pan_session::config::CrateType;
use pan_span::hygiene::{ExpnDataEncodeMode, HygieneEncodeContext, MacroKind};
use pan_span::symbol::{sym, Ident, Symbol};
use pan_span::{self, ExternalSource, FileName, SourceFile, Span, SyntaxContext};
use pan_target::abi::VariantIdx;
use std::hash::Hash;
...
impl<'a, 'tcx> Encodable<EncodeContext<'a, 'tcx>> for SyntaxContext {
    fn encode(&self, s: &mut EncodeContext<'a, 'tcx>) -> opaque::EncodeResult {
        pan_span::hygiene::raw_encode_syntax_context(*self, &s.hygiene_ctxt, s)
    }
}

impl<'a, 'tcx> Encodable<EncodeContext<'a, 'tcx>> for ExpnId {
    fn encode(&self, s: &mut EncodeContext<'a, 'tcx>) -> opaque::EncodeResult {
        pan_span::hygiene::raw_encode_expn_id(
            *self,
            &s.hygiene_ctxt,
...
impl<'a, 'tcx> TyEncoder<'tcx> for EncodeContext<'a, 'tcx> {
    ...
    fn predicate_shorthands(&mut self) -> &mut FxHashMap<pan_middle::ty::Predicate<'tcx>, usize> {
        &mut self.predicate_shorthands
    }

    fn encode_alloc_id(
        &mut self,
        alloc_id: &pan_middle::mir::interpret::AllocId,
    ) -> Result<(), Self::Error> {
...
impl<'a, 'tcx> EncodeContext<'a, 'tcx> {
    ...
    fn encode_source_map(&mut self) -> Lazy<[pan_span::SourceFile]> {
...
impl EncodeContext<'a, 'tcx> {
    ...
    fn encode_info_for_trait_item(&mut self, def_id: DefId) {
        ...
        record!(self.tables.kind[def_id] <- match trait_item.kind {
            ty::AssocKind::Const => {
                let rendered = pan_hir_pretty::to_string(
    ...
    fn encode_rendered_const_for_body(&mut self, body_id: hir::BodyId) -> Lazy<RenderedConst> {
        ...
        let rendered = pan_hir_pretty::to_string(&(&hir as &dyn intravisit::Map<'_>), |s| {
