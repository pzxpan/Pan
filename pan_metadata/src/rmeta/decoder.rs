...
use pan_ast as ast;
use pan_attr as attr;
use pan_data_structures::captures::Captures;
use pan_data_structures::fingerprint::{Fingerprint, FingerprintDecoder};
use pan_data_structures::fx::FxHashMap;
use pan_data_structures::svh::Svh;
use pan_data_structures::sync::{AtomicCell, Lock, LockGuard, Lrc, OnceCell};
use pan_errors::ErrorReported;
use pan_expand::base::{SyntaxExtension, SyntaxExtensionKind};
use pan_expand::proc_macro::{AttrProcMacro, BangProcMacro, ProcMacroDerive};
use pan_hir as hir;
use pan_hir::def::{CtorKind, CtorOf, DefKind, Res};
use pan_hir::def_id::{CrateNum, DefId, DefIndex, CRATE_DEF_INDEX, LOCAL_CRATE};
use pan_hir::definitions::{DefKey, DefPath, DefPathData, DefPathHash};
use pan_hir::lang_items;
use pan_index::vec::{Idx, IndexVec};
use pan_middle::dep_graph::{self, DepNode, DepNodeExt, DepNodeIndex};
use pan_middle::hir::exports::Export;
use pan_middle::middle::cstore::{CrateSource, ExternCrate};
use pan_middle::middle::cstore::{ForeignModule, LinkagePreference, NativeLib};
use pan_middle::middle::exported_symbols::{ExportedSymbol, SymbolExportLevel};
use pan_middle::mir::interpret::{AllocDecodingSession, AllocDecodingState};
use pan_middle::mir::{self, Body, Promoted};
use pan_middle::ty::codec::TyDecoder;
use pan_middle::ty::{self, Ty, TyCtxt};
use pan_serialize::{opaque, Decodable, Decoder};
use pan_session::Session;
use pan_span::hygiene::ExpnDataDecodeMode;
use pan_span::source_map::{respan, Spanned};
use pan_span::symbol::{sym, Ident, Symbol};
use pan_span::{self, hygiene::MacroKind, BytePos, ExpnId, Pos, Span, SyntaxContext, DUMMY_SP};
...
use pan_span::hygiene::HygieneDecodeContext;
...
struct ImportedSourceFile {
    ...
    original_start_pos: pan_span::BytePos,
    /// The end of this SourceFile within the source_map of its original crate
    original_end_pos: pan_span::BytePos,
    /// The imported SourceFile's representation within the local source_map
    translated_source_file: Lrc<pan_span::SourceFile>,
}
...
impl<'a, 'tcx> TyDecoder<'tcx> for DecodeContext<'a, 'tcx> {
    ...
    fn decode_alloc_id(&mut self) -> Result<pan_middle::mir::interpret::AllocId, Self::Error> {
...
impl<'a, 'tcx> Decodable<DecodeContext<'a, 'tcx>> for SyntaxContext {
    fn decode(decoder: &mut DecodeContext<'a, 'tcx>) -> Result<SyntaxContext, String> {
        ...
        pan_span::hygiene::decode_syntax_context(decoder, &cdata.hygiene_context, |_, id| {
...
impl<'a, 'tcx> Decodable<DecodeContext<'a, 'tcx>> for ExpnId {
    fn decode(decoder: &mut DecodeContext<'a, 'tcx>) -> Result<ExpnId, String> {
        ...
        pan_span::hygiene::decode_expn_id(
            decoder,
...
impl<'a, 'tcx> CrateMetadataRef<'a> {
    ...
    fn imported_source_files(&self, sess: &Session) -> &'a [ImportedSourceFile] {
        ...
        let try_to_translate_virtual_to_real = |name: &mut pan_span::FileName| {
            debug!(
            ...
            if let Some(virtual_dir) = virtual_rust_source_base_dir {
                if let Some(real_dir) = &sess.real_rust_source_base_dir {
                    if let pan_span::FileName::Real(old_name) = name {
                        if let pan_span::RealFileName::Named(one_path) = old_name {
                            if let Ok(rest) = one_path.strip_prefix(virtual_dir) {
                                ...
                                let new_name = pan_span::RealFileName::Devirtualized {
                                    local_path: new_path,
                                    virtual_name,
                                };
                                *old_name = new_name;
