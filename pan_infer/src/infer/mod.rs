...
use pan_data_structures::undo_log::Rollback;
use pan_data_structures::unify as ut;
use pan_errors::DiagnosticBuilder;
use pan_hir as hir;
use pan_hir::def_id::{DefId, LocalDefId};
use pan_middle::infer::canonical::{Canonical, CanonicalVarValues};
use pan_middle::infer::unify_key::{ConstVarValue, ConstVariableValue};
use pan_middle::infer::unify_key::{ConstVariableOrigin, ConstVariableOriginKind, ToType};
use pan_middle::mir;
use pan_middle::mir::interpret::EvalToConstValueResult;
use pan_middle::traits::select;
use pan_middle::ty::error::{ExpectedFound, TypeError, UnconstrainedNumeric};
use pan_middle::ty::fold::{TypeFoldable, TypeFolder};
use pan_middle::ty::relate::RelateResult;
use pan_middle::ty::subst::{GenericArg, GenericArgKind, InternalSubsts, SubstsRef};
pub use pan_middle::ty::IntVarValue;
use pan_middle::ty::{self, GenericParamDefKind, InferConst, Ty, TyCtxt};
use pan_middle::ty::{ConstVid, FloatVid, IntVid, TyVid};
use pan_session::config::BorrowckMode;
...
pub use pan_middle::infer::unify_key;
...
impl RegionVariableOrigin {
    pub fn span(&self) -> Span {
        match *self {
            ...
            | LateBoundRegion(a, ..)
            | UpvarRegion(_, a) => a,
            BoundRegionInCoherence(_) => pan_span::DUMMY_SP,
