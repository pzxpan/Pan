...
use pan_middle::ty::{self, DefIdTree, Region, Ty};
use pan_span::Span;
...
impl<'a, 'tcx> NiceRegionError<'a, 'tcx> {
    ...
    pub(super) fn future_return_type(
        ...
        if let Some(hir::IsAsync::Async) = self.asyncness(local_def_id) {
            if let pan_middle::ty::Opaque(def_id, _) =
                        ...
                        for b in bounds.iter() {
                            ...
                            ) = b
                            {
                                for type_binding in generic_args.bindings.iter() {
                                    if type_binding.ident.name == pan_span::sym::Output {
    ...
    pub(super) fn asyncness(&self, local_def_id: LocalDefId) -> Option<hir::IsAsync> {
        ...
        let fn_like = pan_middle::hir::map::blocks::FnLikeNode::from_node(node)?;
