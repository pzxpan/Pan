...
use pan_ast::attr;
use pan_ast::ptr::P;
use pan_ast::token::{self, BinOpToken, CommentKind, DelimToken, Nonterminal, Token, TokenKind};
use pan_ast::tokenstream::{TokenStream, TokenTree};
use pan_ast::util::classify;
use pan_ast::util::comments::{gather_comments, Comment, CommentStyle};
use pan_ast::util::parser::{self, AssocOp, Fixity};
use pan_ast::{self as ast, BlockCheckMode, PatKind, RangeEnd, RangeSyntax};
use pan_ast::{GenericArg, MacArgs};
use pan_ast::{GenericBound, SelfKind, TraitBoundModifier};
use pan_ast::{InlineAsmOperand, InlineAsmRegOrRegClass};
use pan_ast::{InlineAsmOptions, InlineAsmTemplatePiece};
use pan_span::edition::Edition;
use pan_span::source_map::{SourceMap, Spanned};
use pan_span::symbol::{kw, sym, Ident, IdentPrinter, Symbol};
use pan_span::{BytePos, FileName, Span};
...
pub trait PrintState<'a>: std::ops::Deref<Target = pp::Printer> + std::ops::DerefMut {
    ...
    fn bclose_maybe_open(&mut self, span: pan_span::Span, close_box: bool) {
    ...
    fn bclose(&mut self, span: pan_span::Span) {
...
impl<'a> State<'a> {
    ...
    crate fn print_variants(&mut self, variants: &[ast::Variant], span: pan_span::Span) {
        self.bopen();
        for v in variants {
            self.space_if_not_bol();
