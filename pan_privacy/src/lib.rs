...
use pan_hir::def_id::{CrateNum, DefId, LocalDefId, CRATE_DEF_INDEX, LOCAL_CRATE};
use pan_hir::intravisit::{self, DeepVisitor, NestedVisitorMap, Visitor};
use pan_hir::{AssocItemKind, HirIdSet, Node, PatKind};
use pan_middle::bug;
use pan_middle::hir::map::Map;
use pan_middle::middle::privacy::{AccessLevel, AccessLevels};
use pan_middle::span_bug;
use pan_middle::ty::fold::TypeVisitor;
use pan_middle::ty::query::Providers;
use pan_middle::ty::subst::InternalSubsts;
use pan_middle::ty::{self, GenericParamDefKind, TraitRef, Ty, TyCtxt, TypeFoldable};
use pan_session::lint;
use pan_span::hygiene::Transparency;
...
impl<'tcx> Visitor<'tcx> for TypePrivacyVisitor<'tcx> {
    ...
    fn visit_ty(&mut self, hir_ty: &'tcx hir::Ty<'tcx>) {
        ...
        if let Some(typeck_results) = self.maybe_typeck_results {
            ...
            if self.visit(pan_typeck::hir_ty_to_ty(self.tcx, hir_ty)).is_break() {
                return;
            }
        }
    ...
    fn visit_trait_ref(&mut self, trait_ref: &'tcx hir::TraitRef<'tcx>) {
        ...
        if self.maybe_typeck_results.is_none() {
            ...
            let bounds = pan_typeck::hir_trait_to_predicates(
                self.tcx,
                trait_ref,
