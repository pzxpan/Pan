...
use pan_ast as ast;
use pan_ast::util::parser::{self, AssocOp, Fixity};
use pan_ast_pretty::pp::Breaks::{Consistent, Inconsistent};
use pan_ast_pretty::pp::{self, Breaks};
use pan_ast_pretty::pprust::{Comments, PrintState};
use pan_hir as hir;
use pan_hir::{GenericArg, GenericParam, GenericParamKind, Node};
use pan_hir::{GenericBound, PatKind, RangeEnd, TraitBoundModifier};
use pan_span::source_map::{SourceMap, Spanned};
use pan_span::symbol::{kw, Ident, IdentPrinter, Symbol};
use pan_span::{self, BytePos, FileName};
...
pub fn id_to_string(map: &dyn pan_hir::intravisit::Map<'_>, hir_id: hir::HirId) -> String {
...
impl PpAnn for &dyn pan_hir::intravisit::Map<'_> {
    fn nested(&self, state: &mut State<'_>, nested: Nested) {
        match nested {
            Nested::Item(id) => state.print_item(self.item(id.id)),
            Nested::TraitItem(id) => state.print_trait_item(self.trait_item(id)),
...
impl<'a> State<'a> {
    ...
    pub fn bclose_maybe_open(&mut self, span: pan_span::Span, close_box: bool) {
    ...
    pub fn bclose(&mut self, span: pan_span::Span) {
    ...
    pub fn print_variants(&mut self, variants: &[hir::Variant<'_>], span: pan_span::Span) {
        self.bopen();
        for v in variants {
            self.space_if_not_bol();
    ...
    pub fn print_trait_item(&mut self, ti: &hir::TraitItem<'_>) {
        ...
        match ti.kind {
            hir::TraitItemKind::Const(ref ty, default) => {
                let vis =
                    Spanned { span: pan_span::DUMMY_SP, node: hir::VisibilityKind::Inherited };
    ...
    pub fn print_ty_fn(
        ...
        let generics = hir::Generics {
            params: &[],
            where_clause: hir::WhereClause { predicates: &[], span: pan_span::DUMMY_SP },
