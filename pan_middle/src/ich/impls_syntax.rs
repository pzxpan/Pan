...
use pan_span::{BytePos, NormalizedPos, SourceFile};

use smallvec::SmallVec;

impl<'ctx> pan_target::HashStableContext for StableHashingContext<'ctx> {}
...
impl<'ctx> pan_ast::HashStableContext for StableHashingContext<'ctx> {
...
fn stable_multibyte_char(mbc: pan_span::MultiByteChar, source_file_start: BytePos) -> (u32, u32) {
    let pan_span::MultiByteChar { pos, bytes } = mbc;

    (pos.0 - source_file_start.0, bytes as u32)
}

fn stable_non_narrow_char(
    swc: pan_span::NonNarrowChar,
...
impl<'tcx> HashStable<StableHashingContext<'tcx>> for pan_feature::Features {
