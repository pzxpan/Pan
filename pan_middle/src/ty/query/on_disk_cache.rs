...
use pan_data_structures::fingerprint::{Fingerprint, FingerprintDecoder, FingerprintEncoder};
use pan_data_structures::fx::{FxHashMap, FxHashSet, FxIndexSet};
use pan_data_structures::sync::{HashMapExt, Lock, Lrc, OnceCell};
use pan_data_structures::thin_vec::ThinVec;
use pan_errors::Diagnostic;
use pan_hir::def_id::{CrateNum, DefId, DefIndex, LocalDefId, LOCAL_CRATE};
use pan_hir::definitions::DefPathHash;
use pan_hir::definitions::Definitions;
use pan_index::vec::{Idx, IndexVec};
use pan_serialize::{opaque, Decodable, Decoder, Encodable, Encoder};
use pan_session::{CrateDisambiguator, Session};
use pan_span::hygiene::{
    ExpnDataDecodeMode, ExpnDataEncodeMode, ExpnId, HygieneDecodeContext, HygieneEncodeContext,
    SyntaxContext, SyntaxContextData,
};
use pan_span::source_map::{SourceMap, StableSourceFileId};
use pan_span::CachingSourceMapView;
use pan_span::{BytePos, ExpnData, SourceFile, Span, DUMMY_SP};
...
impl<'a, 'tcx> Decodable<CacheDecoder<'a, 'tcx>> for SyntaxContext {
    fn decode(decoder: &mut CacheDecoder<'a, 'tcx>) -> Result<Self, String> {
        ...
        pan_span::hygiene::decode_syntax_context(decoder, decoder.hygiene_context, |this, id| {
...
impl<'a, 'tcx> Decodable<CacheDecoder<'a, 'tcx>> for &'tcx [pan_ast::InlineAsmTemplatePiece] {
    fn decode(d: &mut CacheDecoder<'a, 'tcx>) -> Result<Self, String> {
        RefDecodable::decode(d)
    }
}
...
impl<'a, 'tcx> FingerprintEncoder for CacheEncoder<'a, 'tcx, pan_serialize::opaque::Encoder> {
    ...
    E: 'a + OpaqueEncoder,
{
    fn encode(&self, s: &mut CacheEncoder<'a, 'tcx, E>) -> Result<(), E::Error> {
        pan_span::hygiene::raw_encode_syntax_context(*self, s.hygiene_context, s)
    }
}
