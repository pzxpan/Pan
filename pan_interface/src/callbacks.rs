...
use pan_errors::{Diagnostic, TRACK_DIAGNOSTICS};
use pan_middle::ty::tls;
use std::fmt;

/// This is a callback from librustc_ast as it cannot access the implicit state
/// in librustc_middle otherwise.
fn span_debug(span: pan_span::Span, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    tls::with_opt(|tcx| {
        if let Some(tcx) = tcx {
            pan_span::debug_with_source_map(span, f, tcx.sess.source_map())
        } else {
            pan_span::default_span_debug(span, f)
        }
    })
}
...
/// in librustc_middle otherwise.
fn def_id_debug(def_id: pan_hir::def_id::DefId, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
pub fn setup_callbacks() {
    pan_span::SPAN_DEBUG.swap(&(span_debug as fn(_, &mut fmt::Formatter<'_>) -> _));
    pan_hir::def_id::DEF_ID_DEBUG.swap(&(def_id_debug as fn(_, &mut fmt::Formatter<'_>) -> _));
