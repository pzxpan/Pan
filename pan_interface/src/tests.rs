...
use pan_errors::{emitter::HumanReadableErrorType, registry, ColorConfig};
use pan_session::config::Strip;
use pan_session::config::{build_configuration, build_session_options, to_crate_config};
use pan_session::config::{pan_optgroups, ErrorOutputType, ExternLocation, Options, Passes};
use pan_session::config::{CFGuard, ExternEntry, LinkerPluginLto, LtoCli, SwitchWithOptPath};
use pan_session::config::{
    Externs, OutputType, OutputTypes, SanitizerSet, SymbolManglingVersion,
};
use pan_session::lint::Level;
use pan_session::search_paths::SearchPath;
use pan_session::utils::NativeLibKind;
use pan_session::{build_session, getopts, DiagnosticOutput, Session};
use pan_span::edition::{Edition, DEFAULT_EDITION};
use pan_span::symbol::sym;
use pan_span::SourceFileHashAlgorithm;
use pan_target::spec::{CodeModel, LinkerFlavor, MergeFunctions, PanicStrategy};
use pan_target::spec::{RelocModel, RelroLevel, TlsModel};
...
fn test_switch_implies_cfg_test() {
    pan_span::with_default_session_globals(|| {
