...
use pan_ast as ast;
use pan_ast::util::comments::beautify_doc_string;
use pan_ast_pretty::pprust::attribute_to_string;
use pan_hir as hir;
use pan_hir::def::{DefKind as HirDefKind, Res};
use pan_hir::def_id::{DefId, LOCAL_CRATE};
use pan_hir::intravisit::{self, Visitor};
use pan_hir::Node;
use pan_hir_pretty::{enum_def_to_string, fn_to_string, ty_to_string};
use pan_middle::hir::map::Map;
use pan_middle::middle::cstore::ExternCrate;
use pan_middle::middle::privacy::AccessLevels;
use pan_middle::ty::{self, print::with_no_trimmed_paths, DefIdTree, TyCtxt};
use pan_middle::{bug, span_bug};
use pan_session::config::{CrateType, Input, OutputType};
use pan_session::output::{filename_for_metadata, out_filename};
                ...
                Some(impl_id) => match self.tcx.hir().get_if_local(impl_id) {
                    Some(Node::Item(item)) => match item.kind {
                        hir::ItemKind::Impl { ref self_ty, .. } => {
                            ...
                            qualname
                                .push_str(&pan_hir_pretty::id_to_string(&hir, self_ty.hir_id));
