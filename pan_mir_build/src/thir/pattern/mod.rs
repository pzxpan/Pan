...
use pan_hir::pat_util::EnumerateAndAdjustIterator;
use pan_hir::RangeEnd;
use pan_index::vec::Idx;
use pan_middle::mir::interpret::{get_slice_bytes, ConstValue};
use pan_middle::mir::interpret::{ErrorHandled, LitToConstError, LitToConstInput};
use pan_middle::mir::UserTypeProjection;
use pan_middle::mir::{BorrowKind, Field, Mutability};
use pan_middle::ty::subst::{GenericArg, SubstsRef};
use pan_middle::ty::{self, AdtDef, DefIdTree, Region, Ty, TyCtxt, UserType};
use pan_middle::ty::{
    CanonicalUserType, CanonicalUserTypeAnnotation, CanonicalUserTypeAnnotations,
};
use pan_span::{Span, Symbol, DUMMY_SP};
...
crate fn compare_const_vals<'tcx>(
    ...
    if let (Some(a), Some(b)) = (a_bits, b_bits) {
        use pan_apfloat::Float;
        return match *ty.kind() {
            ty::Float(ast::FloatTy::F32) => {
                let l = pan_apfloat::ieee::Single::from_bits(a);
                let r = pan_apfloat::ieee::Single::from_bits(b);
                l.partial_cmp(&r)
            }
            ty::Float(ast::FloatTy::F64) => {
                let l = pan_apfloat::ieee::Double::from_bits(a);
                let r = pan_apfloat::ieee::Double::from_bits(b);
                l.partial_cmp(&r)
            }
            ty::Int(ity) => {
                use pan_attr::SignedInt;
                use pan_middle::ty::layout::IntegerExt;
                let size = pan_target::abi::Integer::from_attr(&tcx, SignedInt(ity)).size();
