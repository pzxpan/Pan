...
use pan_ast as ast;
use pan_ast::{MetaItemKind, NestedMetaItem};
use pan_attr::{list_contains_name, InlineAttr, InstructionSetAttr, OptimizeAttr};
use pan_data_structures::captures::Captures;
use pan_data_structures::fx::{FxHashMap, FxHashSet, FxIndexSet};
use pan_errors::{struct_span_err, Applicability};
use pan_hir as hir;
use pan_hir::def::{CtorKind, DefKind, Res};
use pan_hir::def_id::{DefId, LocalDefId, LOCAL_CRATE};
use pan_hir::intravisit::{self, NestedVisitorMap, Visitor};
use pan_hir::weak_lang_items;
use pan_hir::{GenericParamKind, HirId, Node};
use pan_middle::hir::map::blocks::FnLikeNode;
use pan_middle::hir::map::Map;
use pan_middle::middle::codegen_fn_attrs::{CodegenFnAttrFlags, CodegenFnAttrs};
use pan_middle::mir::mono::Linkage;
use pan_middle::ty::query::Providers;
use pan_middle::ty::subst::InternalSubsts;
use pan_middle::ty::util::Discr;
use pan_middle::ty::util::IntTypeExt;
use pan_middle::ty::{self, AdtKind, Const, DefIdTree, ToPolyTraitRef, Ty, TyCtxt};
use pan_middle::ty::{ReprOptions, ToPredicate, WithConstness};
use pan_session::config::SanitizerSet;
use pan_session::lint;
use pan_session::parse::feature_err;
use pan_span::symbol::{kw, sym, Ident, Symbol};
use pan_span::{Span, DUMMY_SP};
use pan_target::spec::abi;
use pan_trait_selection::traits::error_reporting::suggestions::NextTypeParamName;
...
fn bad_placeholder_type(
    tcx: TyCtxt<'tcx>,
    mut spans: Vec<Span>,
) -> pan_errors::DiagnosticBuilder<'tcx> {
    spans.sort();
...
fn generator_kind(tcx: TyCtxt<'_>, def_id: DefId) -> Option<hir::GeneratorKind> {
    match tcx.hir().get_if_local(def_id) {
        Some(Node::Expr(&pan_hir::Expr {
            kind: pan_hir::ExprKind::Closure(_, _, body_id, _, _),
            ..
...
fn check_link_ordinal(tcx: TyCtxt<'_>, attr: &ast::Attribute) -> Option<usize> {
    use pan_ast::{Lit, LitIntType, LitKind};
