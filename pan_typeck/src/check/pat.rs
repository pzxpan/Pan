...
use pan_hir::{HirId, Pat, PatKind};
use pan_infer::infer;
use pan_infer::infer::type_variable::{TypeVariableOrigin, TypeVariableOriginKind};
use pan_middle::ty::subst::GenericArg;
use pan_middle::ty::{self, Adt, BindingMode, Ty, TypeFoldable};
...
use pan_trait_selection::traits::{ObligationCause, Pattern};
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    fn error_tuple_variant_as_struct_pat(
        ...
        if let (CtorKind::Fn, PatKind::Struct(qpath, ..)) = (variant.ctor_kind, &pat.kind) {
            let path = pan_hir_pretty::to_string(pan_hir_pretty::NO_ANN, |s| {
                ...
                (
                    fields
                        ...
                        .map(|f| match self.tcx.sess.source_map().span_to_snippet(f.pat.span) {
                            Ok(f) => f,
                            Err(_) => pan_hir_pretty::to_string(pan_hir_pretty::NO_ANN, |s| {
                                s.print_pat(f.pat)
                            }),
                        })
                        .collect::<Vec<String>>()
