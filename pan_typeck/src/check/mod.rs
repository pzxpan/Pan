...
use pan_errors::{pluralize, struct_span_err, Applicability};
use pan_hir as hir;
use pan_hir::def::Res;
use pan_hir::def_id::{CrateNum, DefId, LocalDefId, LOCAL_CRATE};
use pan_hir::intravisit::Visitor;
use pan_hir::itemlikevisit::ItemLikeVisitor;
use pan_hir::{HirIdMap, ImplicitSelfKind, Node};
...
use pan_middle::ty::WithConstness;
use pan_middle::ty::{self, RegionKind, Ty, TyCtxt, UserType};
use pan_session::config;
use pan_session::parse::feature_err;
use pan_session::Session;
use pan_span::source_map::DUMMY_SP;
use pan_span::symbol::{kw, Ident};
use pan_span::{self, BytePos, MultiSpan, Span};
use pan_target::abi::VariantIdx;
use pan_target::spec::abi::Abi;
use pan_trait_selection::traits;
use pan_trait_selection::traits::error_reporting::recursive_type_with_infinite_size_error;
use pan_trait_selection::traits::error_reporting::suggestions::ReturnsVisitor;
...
macro_rules! type_error_struct {
    ($session:expr, $span:expr, $typ:expr, $code:ident, $($message:tt)*) => ({
        if $typ.references_error() {
            ...
            pan_errors::struct_span_err!($session, $span, $code, $($message)*)
        }
    })
}
