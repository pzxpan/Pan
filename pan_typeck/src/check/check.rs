...
use pan_errors::{Applicability, ErrorReported};
use pan_hir as hir;
use pan_hir::def_id::{DefId, LocalDefId, LOCAL_CRATE};
use pan_hir::lang_items::LangItem;
use pan_hir::{ItemKind, Node};
use pan_infer::infer::type_variable::{TypeVariableOrigin, TypeVariableOriginKind};
use pan_infer::infer::{RegionVariableOrigin, TyCtxtInferExt};
use pan_middle::ty::fold::TypeFoldable;
use pan_middle::ty::subst::GenericArgKind;
use pan_middle::ty::util::{Discr, IntTypeExt, Representability};
use pan_middle::ty::{self, ParamEnv, RegionKind, ToPredicate, Ty, TyCtxt};
use pan_session::config::EntryFnType;
use pan_session::lint::builtin::UNINHABITED_STATIC;
use pan_span::symbol::sym;
use pan_span::{self, MultiSpan, Span};
...
use pan_trait_selection::traits::{self, ObligationCauseCode};
            ...
            for item in items.iter() {
                ...
                match item.kind {
                    ...
                    hir::TraitItemKind::Type(.., Some(_default)) => {
                        ...
                        let trait_substs =
                            InternalSubsts::identity_for_item(tcx, def_id.to_def_id());
                        let _: Result<_, pan_errors::ErrorReported> = check_type_bounds(
                            tcx,
                            assoc_item,
                            assoc_item,
                            item.span,
