...
pub extern crate pan_plugin_impl as plugin;

use pan_ast as ast;
use pan_codegen_ssa::{traits::CodegenBackend, CodegenResults};
use pan_data_structures::profiling::print_time_passes_entry;
use pan_data_structures::sync::SeqCst;
use pan_errors::registry::{InvalidErrorCode, Registry};
use pan_errors::{ErrorReported, PResult};
use pan_feature::find_gated_cfg;
use pan_hir::def_id::LOCAL_CRATE;
use pan_interface::util::{self, collect_crate_types, get_builtin_codegen_backend};
use pan_interface::{interface, Queries};
use pan_lint::LintStore;
use pan_metadata::locator;
use pan_middle::middle::cstore::MetadataLoader;
use pan_middle::ty::TyCtxt;
use pan_save_analysis as save;
use pan_save_analysis::DumpHandler;
use pan_serialize::json::{self, ToJson};
use pan_session::config::nightly_options;
use pan_session::config::{ErrorOutputType, Input, OutputType, PrintRequest, TrimmedDefPaths};
use pan_session::getopts;
use pan_session::lint::{Lint, LintId};
use pan_session::{config, DiagnosticOutput, Session};
use pan_session::{early_error, early_warn};
use pan_span::source_map::{FileLoader, FileName};
...
pub fn diagnostics_registry() -> Registry {
    Registry::new(&pan_error_codes::DIAGNOSTICS)
}
...
impl RustcDefaultCalls {
    ...
    fn print_crate_info(
        ...
        use pan_session::config::PrintRequest::*;
        ...
        for req in &sess.opts.prints {
            match *req {
                TargetList => {
                    let mut targets =
                        pan_target::spec::TARGETS.iter().copied().collect::<Vec<_>>();
                ...
                FileNames | CrateName => {
                    ...
                    let t_outputs = pan_interface::util::build_output_filenames(
                        input, odir, ofile, attrs, sess,
                    );
                    let id = pan_session::output::find_crate_name(sess, attrs, input);
                    ...
                    for &style in &crate_types {
                        let fname =
                            pan_session::output::filename_for_input(sess, style, &id, &t_outputs);
...
fn usage(verbose: bool, include_unstable_options: bool, nightly_build: bool) {
    ...
    let message = "Usage: pan [OPTIONS] INPUT";
    ...
    let verbose_help = if verbose {
        ...
        "\n    --help -v           Print the full set of options pan accepts"
    };
...
fn print_wall_help() {
    println!(
        "
The flag `-Wall` does not exist in `pan`. Most useful lints are enabled by
default. Use `pan -W help` to see all available lints. It's more common to put
...
"
    ...
    println!("Lint checks provided by pan:\n");
    ...
    println!("Lint groups provided by pan:\n");
...
pub fn handle_options(args: &[String]) -> Option<getopts::Matches> {
    ...
    if args.is_empty() {
        ...
        let nightly_build =
            pan_feature::UnstableFeatures::from_environment(None).is_nightly_build();
...
fn parse_crate_attrs<'a>(sess: &'a Session, input: &Input) -> PResult<'a, Vec<ast::Attribute>> {
    match input {
        Input::File(ifile) => pan_parse::parse_crate_attrs_from_file(ifile, &sess.parse_sess),
        Input::Str { name, input } => pan_parse::parse_crate_attrs_from_source_str(
            name.clone(),
            input.clone(),
            &sess.parse_sess,
        ),
...
pub fn catch_fatal_errors<F: FnOnce() -> R, R>(f: F) -> Result<R, ErrorReported> {
    catch_unwind(panic::AssertUnwindSafe(f)).map_err(|value| {
        if value.is::<pan_errors::FatalErrorMarker>() {
            ErrorReported
        } else {
...
pub fn report_ice(info: &panic::PanicInfo<'_>, bug_report_url: &str) {
    ...
    let emitter = Box::new(pan_errors::emitter::EmitterWriter::stderr(
        pan_errors::ColorConfig::Auto,
        None,
        false,
        false,
        None,
    ...
    let handler = pan_errors::Handler::with_emitter(true, None, emitter);

    // a .span_bug or .bug call has already printed what
    // it wants to print.
    if !info.payload().is::<pan_errors::ExplicitBug>() {
        let d = pan_errors::Diagnostic::new(pan_errors::Level::Bug, "unexpected panic");
