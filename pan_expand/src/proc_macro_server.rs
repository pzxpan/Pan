...
use pan_ast as ast;
use pan_ast::token;
use pan_ast::tokenstream::{self, CanSynthesizeMissingTokens};
use pan_ast::tokenstream::{DelimSpan, Spacing::*, TokenStream, TreeAndSpacing};
use pan_ast_pretty::pprust;
use pan_data_structures::sync::Lrc;
use pan_errors::Diagnostic;
use pan_parse::lexer::nfc_normalize;
use pan_parse::{nt_to_tokenstream, parse_stream_from_source_str};
use pan_session::parse::ParseSess;
use pan_span::symbol::{self, kw, sym, Symbol};
use pan_span::{BytePos, FileName, MultiSpan, Pos, SourceFile, Span};
...
impl ToInternal<pan_errors::Level> for Level {
    fn to_internal(self) -> pan_errors::Level {
        match self {
            Level::Error => pan_errors::Level::Error,
            Level::Warning => pan_errors::Level::Warning,
            Level::Note => pan_errors::Level::Note,
            Level::Help => pan_errors::Level::Help,
...
impl server::TokenStreamBuilder for Pan<'_> {
...
impl server::TokenStreamIter for Pan<'_> {
    fn next(
        &mut self,
...
impl server::SourceFile for Pan<'_> {
...
impl server::MultiSpan for Pan<'_> {
...
impl server::Diagnostic for Pan<'_> {
