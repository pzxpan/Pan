...
use pan_parse::{new_parser_from_source_str, parser::Parser, source_file_to_stream};
use pan_session::parse::ParseSess;
use pan_span::source_map::{FilePathMapping, SourceMap};
use pan_span::with_default_session_globals;
use pan_span::{BytePos, MultiSpan, Span};

use pan_data_structures::sync::Lrc;
use pan_errors::emitter::EmitterWriter;
use pan_errors::{Handler, PResult};
...
crate fn matches_codepattern(a: &str, b: &str) -> bool {
    ...
    loop {
        ...
        if pan_lexer::is_whitespace(a) && pan_lexer::is_whitespace(b) {
            // Skip whitespace for `a` and `b`.
            scan_for_non_ws_or_end(&mut a_iter);
            scan_for_non_ws_or_end(&mut b_iter);
        } else if pan_lexer::is_whitespace(a) {
    ...
    a_iter.all(pan_lexer::is_whitespace)
}

/// Advances the given peekable `Iterator` until it reaches a non-whitespace character.
fn scan_for_non_ws_or_end<I: Iterator<Item = char>>(iter: &mut Peekable<I>) {
    while iter.peek().copied().map(|c| pan_lexer::is_whitespace(c)) == Some(true) {
        iter.next();
    }
}
