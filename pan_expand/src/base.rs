...
use pan_ast::ptr::P;
use pan_ast::token::{self, Nonterminal};
use pan_ast::tokenstream::{CanSynthesizeMissingTokens, TokenStream};
use pan_ast::visit::{AssocCtxt, Visitor};
use pan_ast::{self as ast, Attribute, NodeId, PatKind};
use pan_attr::{self as attr, Deprecation, HasAttrs, Stability};
use pan_data_structures::fx::FxHashMap;
use pan_data_structures::sync::{self, Lrc};
use pan_errors::{DiagnosticBuilder, ErrorReported};
use pan_parse::{self, nt_to_tokenstream, parser, MACRO_ARGUMENTS};
use pan_session::{parse::ParseSess, Limit, Session};
use pan_span::def_id::{DefId, LOCAL_CRATE};
use pan_span::edition::Edition;
use pan_span::hygiene::{AstPass, ExpnData, ExpnId, ExpnKind};
use pan_span::source_map::SourceMap;
use pan_span::symbol::{kw, sym, Ident, Symbol};
use pan_span::{FileName, MultiSpan, Span, DUMMY_SP};
...
crate use pan_span::hygiene::MacroKind;
...
impl<'a> ExtCtxt<'a> {
    ...
    pub fn new_parser_from_tts(&self, stream: TokenStream) -> parser::Parser<'a> {
        pan_parse::stream_to_parser(&self.sess.parse_sess, stream, MACRO_ARGUMENTS)
    }
