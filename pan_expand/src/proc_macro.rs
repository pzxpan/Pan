...
use pan_ast::ptr::P;
use pan_ast::token;
use pan_ast::tokenstream::{CanSynthesizeMissingTokens, TokenStream, TokenTree};
use pan_ast::{self as ast, *};
use pan_data_structures::sync::Lrc;
use pan_errors::{struct_span_err, Applicability, ErrorReported};
use pan_lexer::is_ident;
use pan_parse::nt_to_tokenstream;
...
impl base::ProcMacro for BangProcMacro {
    fn expand<'cx>(
        ...
        let server = proc_macro_server::Pan::new(ecx);
...
impl MultiItemModifier for ProcMacroDerive {
    fn expand(
        ...
        let mut parser =
            pan_parse::stream_to_parser(&ecx.sess.parse_sess, stream, Some("proc-macro derive"));
        let mut items = vec![];
