...
use pan_ast::ptr::P;
use pan_ast::token::{self, Token};
use pan_ast::{self as ast, AngleBracketedArg, AngleBracketedArgs, ParenthesizedArgs};
use pan_ast::{AnonConst, AssocTyConstraint, AssocTyConstraintKind, BlockCheckMode};
use pan_ast::{GenericArg, GenericArgs};
use pan_ast::{Path, PathSegment, QSelf};
use pan_errors::{pluralize, Applicability, PResult};
use pan_span::source_map::{BytePos, Span};
...
impl<'a> Parser<'a> {
    ...
    pub(super) fn expr_is_valid_const_arg(&self, expr: &P<pan_ast::Expr>) -> bool {
        match &expr.kind {
