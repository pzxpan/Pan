...
use pan_ast::ptr::P;
use pan_ast::token::{self, Token, TokenKind};
use pan_ast::tokenstream::Spacing;
use pan_ast::util::classify;
use pan_ast::util::literal::LitError;
use pan_ast::util::parser::{prec_let_scrutinee_needs_par, AssocOp, Fixity};
use pan_ast::{self as ast, AttrStyle, AttrVec, CaptureBy, Field, Lit, UnOp, DUMMY_NODE_ID};
use pan_ast::{AnonConst, BinOp, BinOpKind, FnDecl, FnRetTy, MacCall, Param, Ty, TyKind};
use pan_ast::{Arm, Async, BlockCheckMode, Expr, ExprKind, Label, Movability, RangeLimits};
use pan_ast_pretty::pprust;
use pan_errors::{Applicability, DiagnosticBuilder, PResult};
use pan_span::edition::LATEST_STABLE_EDITION;
use pan_span::source_map::{self, Span, Spanned};
use pan_span::symbol::{kw, sym, Ident, Symbol};
use pan_span::{BytePos, Pos};
use std::mem;
