...
pub use pan_ast::attr::MarkedAttrs;
pub use pan_ast::crate_disambiguator::CrateDisambiguator;
pub use pan_ast::Attribute;
use pan_data_structures::flock;
use pan_data_structures::fx::{FxHashMap, FxHashSet};
use pan_data_structures::jobserver::{self, Client};
use pan_data_structures::profiling::{duration_to_secs_str, SelfProfiler, SelfProfilerRef};
use pan_data_structures::sync::{
    self, AtomicU64, AtomicUsize, Lock, Lrc, OnceCell, OneThread, Ordering, Ordering::SeqCst,
};
use pan_errors::annotate_snippet_emitter_writer::AnnotateSnippetEmitterWriter;
use pan_errors::emitter::{Emitter, EmitterWriter, HumanReadableErrorType};
use pan_errors::json::JsonEmitter;
use pan_errors::registry::Registry;
use pan_errors::{Applicability, Diagnostic, DiagnosticBuilder, DiagnosticId, ErrorReported};
use pan_lint_defs::FutureBreakage;
use pan_span::edition::Edition;
use pan_span::source_map::{FileLoader, MultiSpan, RealFileLoader, SourceMap, Span};
use pan_span::{sym, SourceFileHashAlgorithm, Symbol};
use pan_target::asm::InlineAsmArch;
use pan_target::spec::{CodeModel, PanicStrategy, RelocModel, RelroLevel};
use pan_target::spec::{Target, TargetTriple, TlsModel};
...
pub struct Session {
    ...
    features: OnceCell<pan_feature::Features>,
...
impl Session {
    ...
    pub fn diagnostic(&self) -> &pan_errors::Handler {
    ...
    #[inline]
    pub fn features_untracked(&self) -> &pan_feature::Features {
        self.features.get().unwrap()
    }

    pub fn init_features(&self, features: pan_feature::Features) {
...
fn default_emitter(
    ...
    registry: pan_errors::registry::Registry,
...
pub fn build_session(
    ...
    let span_diagnostic = pan_errors::Handler::with_emitter_and_flags(
        emitter,
